...
class Payments extends CI_Controller
{
    ...
    public function ipn()
    {
        ...
        $ipn_data = json_decode($request_body, true);

if ($ipn_data) {

    if ($ipn_data['pp_ResponseCode'] === '121') {
            
        
            $isValidTransNo = $this->job_model->validateTransaction($ipn_data);
            if($isValidTransNo)
            {
                $txnDateTime = $ipn_data['pp_TxnDateTime'];
                ...
                $data = [
                    ...
                    'ipn_pp_TxnType' => $ipn_data['pp_TxnType'],
                    'ipn_pp_responseCode' => $ipn_data['pp_ResponseCode'],
                    'ipn_pp_responseMessage' => $ipn_data['pp_ResponseMessage'],
                    'ipn_pp_response' =>  json_encode($ipn_data),
                    'pp_TxnRefNo' => $ipn_data['pp_TxnRefNo'],
                    'status_id' => 16
                ];
                ...
                echo json_encode([
                   "pp_ResponseCode" => "999",
                   "pp_ResponseMessage" => "In Valid transaction id",
                   "pp_SecureHash" => ""
                ]);
                exit;
            }
        ...
        echo json_encode([
         ...
         exit;
        //('ipn_logs.txt', date('Y-m-d H:i:s') . " - Transaction failed: " . $ipn_data['pp_TxnRefNo'] . PHP_EOL, FILE_APPEND);
    }
} else {
    $data = [
        'ipn_payment_date' => "",
    ...
    echo json_encode([
       "pp_ResponseCode" => "999",
        "pp_ResponseMessage" => "Invalid IPN data",
        "pp_SecureHash" => ""
    ]);
     exit;
}
 ...
 exit;
    ...
    public function response()
    {
        ...
        if ($decodedResponse) {
            ...
            if ($decodedResponse['pp_ResponseCode'] === "000") {
                ...
                $data = [
                    ...
                    'bank_name' => 'Card Payment',
                    'job_application_id' => $decodedResponse['pp_BillReference'],
                    'company_name' => $decodedResponse['ppmpf_3'],
                    'job_position' => $decodedResponse['ppmpf_4'],
                ...
                // Restore session from the database
                $session_data = $this->db
                    ->select('session_data')
                    ->get_where('payment_sessions', ['transaction_id' => $decodedResponse['pp_BillReference']])
                    ->row_array();
    
                if ($session_data) {
                    $session_array = unserialize($session_data['session_data']);
                ...
                if ($response) {
                    ...
                    set_alert('error', 'Failed to update payment. Please contact support.');
                }
                ...
                // Restore session in case of failure as well
                $session_data = $this->db
                    ->select('session_data')
                    ->get_where('payment_sessions', ['transaction_id' => $decodedResponse['pp_BillReference']])
                    ->row_array();
            ...
            set_alert('error', 'Invalid response received.');
            redirect(base_url('job/challans'));
        }
    }
