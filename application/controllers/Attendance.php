...
class Attendance extends Admin_Controller
{
    ...
    public function index()
    {
     ...
     public function branchwise_report()
    {
        ...
        if($branchID=='')
        {
            $query = "select concat(f.first_name,' ',f.last_name) as name,d.name as class_name, e.name as section_name,b.name as branch_name, c.roll ,a.status as att_status,  b.name as branch_name
                    from student_attendance a inner join branch b on a.branch_id = b.id inner join enroll c on a.student_id= c.student_id inner join class d on c.class_id = d.id INNER join section e on c.section_id = e.id INNER join student f on f.id = a.student_id
                where a.date = CURRENT_DATE order by a.status,a.branch_id,c.class_id,c.section_id  asc";
                   
        }
        else{
            $query = "select concat(f.first_name,' ',f.last_name) as name,d.name as class_name, e.name as section_name,b.name as branch_name c.roll ,a.status as att_status, b.name as branch_name
 from student_attendance a inner join branch b on a.branch_id = b.id inner join enroll c on a.student_id= c.student_id inner join class d on c.class_id = d.id INNER join section e on c.section_id = e.id INNER join student f on f.id = a.student_id
                where a.date = CURRENT_DATE and b.id =".$branchID." order by a.status,a.branch_id,c.class_id,c.section_id asc";
            
        }
        $results =  $this->db->query($query)->result_array();
        $this->data['branch_id'] = $branchID;
        $this->data['title'] = 'Branch Wise Attendance';
        $this->data['sub_page'] = 'attendance/branch_wise_attendance';
        $this->data['main_menu'] = 'attendance_report';
        $this->data['students'] = $results;
