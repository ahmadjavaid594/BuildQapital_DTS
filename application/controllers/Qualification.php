...
class Qualification extends Admin_Controller
{
        ...
        if (is_superadmin_loggedin()) {
            if ($this->input->post('submit') == 'save') {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    $response = $this->qualification_model->save($post);
                    if ($response) {
            ...
            $this->data['title'] = 'Qualification';
            $this->data['sub_page'] = 'qualification/add';
            $this->data['main_menu'] = 'qualification';
    ...
    public function edit($id = '')
    {
        if (is_superadmin_loggedin()) {
            if ($this->input->post('submit') == 'save') {
              ...
              if ($this->form_validation->run() == true) {
                    $post = $this->input->post();
                    $response = $this->qualification_model->save($post, $id);
                    if ($response) {
            ...
            $this->data['data'] = $this->qualification_model->getSingle('qualification', $id, true);
       
            $this->data['title'] = 'Qualification';
            $this->data['sub_page'] = 'qualification/edit';
            $this->data['main_menu'] = 'qualification';
    ...
    public function unique_name($name)
    {
        ...
        if (!empty($qualification_id)) {
            $this->db->where_not_in('id', $qualification_id);
        }
        $this->db->where('name', $name);
        $name = $this->db->get('qualification')->num_rows();
        if ($name == 0) {
            return true;
        } else {
