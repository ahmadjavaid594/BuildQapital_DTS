        ...
        if (!is_applicant_loggedin()) {
            if ($this->input->post('submit') == 'save' && is_superadmin_loggedin()) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    $response = $this->designation_model->save($post);
                    if ($response) {
            ...
            $this->data['sub_page'] = 'designation/add';
            $this->data['main_menu'] = 'designation';
    ...
    public function edit($id = '')
    {
        if (is_superadmin_loggedin()) {
            if ($this->input->post('submit') == 'save') {
              ...
              if ($this->form_validation->run() == true) {
                    $post = $this->input->post();
                    $response = $this->designation_model->save($post, $id);
                    if ($response) {
            ...
            $this->data['data'] = $this->designation_model->getSingle('designation', $id, true);
       
            $this->data['title'] = translate('designation');
            $this->data['sub_page'] = 'designation/edit';
            $this->data['main_menu'] = 'designation';
