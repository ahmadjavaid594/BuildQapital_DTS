...
//both super admin and school admin
function is_superadmin_loggedin()
{
    $CI = &get_instance();
    if ($CI->session->userdata('loggedin_role_id') == 1 || $CI->session->userdata('loggedin_role_id') == 8 ) {
        return true;
    }
    return false;
}
...
{
    $CI = &get_instance();
    if ($CI->session->userdata('loggedin_role_id') == 10) {
        return true;
    }
    return false;
}
// only super admin
function is_superadmin_only()
{
    $CI = &get_instance();
