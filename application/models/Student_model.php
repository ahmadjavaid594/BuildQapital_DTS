...
//$this->db->select('s.*,l.active,e.class_id,e.section_id,e.id as enrollid,e.roll,e.branch_id,e.session_id,c.name as class_name,se.name as section_name,sc.name as category_name');
        ...
        $this->db->select('id');
        $this->db->from('parent as p');
        $this->db->where('p.branch_id', $branchID);
        $this->db->where('p.email', $row['GuardianEmail']);
        
        $getParent = $this->db->get()->row_array();
    ...
    {
        $this->db->select('e.*,b.name as branch_name,s.photo, CONCAT(s.first_name, " ", s.last_name) as fullname,s.register_no,s.parent_id,s.email,s.blood_group,s.birthday,l.active,c.name as class_name,se.name as section_name ,"student" as entity');
        $this->db->from('enroll as e');
        $this->db->join('student as s', 'e.student_id = s.id', 'inner');
        $this->db->join('login_credential as l', 'l.user_id = s.id and l.role = 7', 'inner');
        $this->db->join('class as c', 'e.class_id = c.id', 'left');
        $this->db->join('section as se', 'e.section_id=se.id', 'left');
         $this->db->join('branch as b', 'e.branch_id=b.id', 'inner');
