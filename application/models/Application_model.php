...
class Application_model extends CI_Model
{
    ...
    public function jobAddUpload()
    {
        if (isset($_FILES["job_add_file"]) && !empty($_FILES['job_add_file']['name'])) {
            $file_size = $_FILES["job_add_file"]["size"];
            $file_name = $_FILES["job_add_file"]["name"];
            $allowedExts = array('pdf');
            $extension = pathinfo($file_name, PATHINFO_EXTENSION);
            if ($files = filesize($_FILES['job_add_file']['tmp_name'])) {
                if (!in_array(strtolower($extension), $allowedExts)) {
                    $this->form_validation->set_message('handle_upload', translate('this_file_type_is_not_allowed'));
                    return false;
                }
    ...
    {
        $this->db->select('e.*,s.photo, CONCAT(s.first_name, " ", s.last_name) as fullname,s.register_no,s.parent_id,s.email,s.mobileno,s.blood_group,s.birthday,s.admission_date,l.active,c.name as class_name,se.name as section_name,  "student" as entity');
